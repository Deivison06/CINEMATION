...
<div class="mt-12 pl-24 pr-24">
    <span class="font-inter font-bold text-xl">Top 10 TV Show</span>

    <div class="flex flex-row overflow-x-auto pt-6 pb-10">
   @foreach ($topShows as $topShowItem)
        @php
            $original_data = $topShowItem->first_air_date;
            $timestamp = strtotime($original_data);
            $movieYear = date('Y', $timestamp);

            $movieId = $topShowItem->id;
            $movieTitle = $topShowItem->original_name;
            $movieRating = $topShowItem->vote_average;
            $bannerImage = "{$imageBaseURL}/w500{$topShowItem->poster_path}";
        @endphp
