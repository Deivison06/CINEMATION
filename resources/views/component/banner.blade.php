...
<div class="w-full h-[512px] flex flex-col relative bg-black">
    ...
    @endphp
        ...
        <div class="flex flex-row items-center w-full h-full relative slide">
          ...
          <div class="w-10/12 flex flex-col ml-28 z-10">
            ...
            <p class="font-inter text-xl text-white w-1/2 line-clamp-2">{{$bannerItem->overview}}</p>
    ...
    <div class="absolute bottom-0 w-full mb-8">
      <div class="w-full flex flex-row items-center justify-center">
        @for ($pos = 1; $pos <= count($banner); $pos++)
          <div class="w-2.5 h-2.5 rounded-full mx-1 cursor-pointer dot bg-develobe-500" onclick="currentSlide(1)"></div>
        @endfor
      </div>
    </div>
  ...
  <script>
    ...
    function showSlide(position) {
      ...
      const dostsArray = document.getElementsByClassName('dot')


      if (position > slideArray.length) {
        slideIndex = 1;
      }
      ...
      for (i = 0; i < slideArray.length; i++) {
        slideArray[i].classList.add('hidden');
      }

      slideArray[slideIndex -1].classList.remove('hidden');

      for (i = 0; i < dostsArray.length; i++) {
        dostsArray[i].classList.remove('bg-develobe-500');
        dostsArray[i].classList.add('bg-white');
      }
      dostsArray[slideIndex - 1].classList.remove('bg-white');
      dostsArray[slideIndex - 1].classList.add('bg-develobe-500');
    }
